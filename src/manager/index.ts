...
export class Manager {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development' || this.devMode) {
        ...
        if (version || pluginLockVersion === 'latest') {
          const newLockVersion = this.getVersion(importPath)
          this.logger.info(
        ...
        pluginInstance = this.pluginManager.require(importPath)
      }
    } catch (error: any) {
      ...
      throw new Error(
        `${this.pluginType} ${plugin} moudle check ${chalk.red(
          'FAILED'
  ...
  getVersion(importPath: string): string {
    const pluginInfo = this.pluginManager.require(`${importPath}/package.json`)
  ...
  async checkRequiredVersion(importPath: string): Promise<boolean> {
    ...
    if (process.env.NODE_ENV === 'development' || this.devMode) {
      ...
      pluginInfo = this.pluginManager.require(`${importPath}/package.json`)
    }
...
Run ${chalk.italic.green('cg update')} to install`
  ...
  writeVersionToLockFile(plugin: string, version: string): void {
    const lockPath = path.join(
      this.cliConfig.configDir,
      '.cloud-graph.lock.json'
    )
