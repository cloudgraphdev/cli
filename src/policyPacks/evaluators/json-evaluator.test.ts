...
describe('JsonEvaluator', () => {
  ...
  it('should execute simple rules', async () => {
    ...
    const expected = []
    for (let r of rules) {
      const res = await e.evaluateSingleResource({conditions: r} as any, {data} as any)
      results.push(res)
  ...
  it('should combine simple rules', async () => {
    ...
    expect(
      await e.evaluateSingleResource({conditions: {and: [trueRule, trueRule, trueRule]}} as any, {data} as any)
    ).toBe(RuleResult.MATCHES)
    expect(
      await e.evaluateSingleResource({conditions: {and: [trueRule, trueRule, falseRule]}} as any, {data} as any)
    ).toBe(RuleResult.DOESNT_MATCH)

    expect(
      await e.evaluateSingleResource({conditions: {or: [falseRule, falseRule, falseRule]}} as any, {data} as any)
    ).toBe(RuleResult.DOESNT_MATCH)
    expect(
      await e.evaluateSingleResource({conditions: {or: [falseRule, trueRule, falseRule]}} as any, {data} as any)
    ).toBe(RuleResult.MATCHES)

    // nested
    expect(
      await e.evaluateSingleResource({
        conditions: {or: [falseRule, falseRule, {and: [trueRule, trueRule]}]}
      } as any, {data} as any)
    ).toBe(RuleResult.MATCHES)

    expect(
      await e.evaluateSingleResource({
        conditions: {or: [falseRule, falseRule, {and: [trueRule, falseRule]}]}
      } as any, {data} as any)
    ).toBe(RuleResult.DOESNT_MATCH)
  })
  ...
  it('should resolve paths', async () => {
    ...
    rule.path = 'a.b[1].d'
    expect(await e.evaluateSingleResource({conditions: rule} as any, data)).toBe(RuleResult.MATCHES)
    rule.path = 'a.b[0].d'
    expect(await e.evaluateSingleResource({conditions: rule} as any, data)).toBe(RuleResult.DOESNT_MATCH)
  ...
  it('should support array operators', async () => {
    ...
    const rule = {path: 'a.b', op: 'array_any', value: {path: '[*]', op: 'equal', value: 2} }
