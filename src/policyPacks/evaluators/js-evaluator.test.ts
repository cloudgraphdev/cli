...
describe('JsEvaluator', ()=> {
  ...
  it('should accept all rules that have a check field', ()=> {
    ...
    expect(e.canEvaluate({check: ()=> {}} as any)).toBe(true)
  })
  ...
  it('should return fail if rule is true', async ()=> {
    ...
    expect(await e.evaluateSingleResource({check: spy} as any, 0 as any)).toEqual(RuleResult.DOESNT_MATCH)
    spy.mockReturnValue(true)
    expect(await e.evaluateSingleResource({check: spy} as any, 0 as any)).toEqual(RuleResult.MATCHES)
